...
alter table users add constraint users_unique2 unique(u_prenom,u_nom);
alter table users add constraint users_check1 check(u_type in ('user','admin','root'));


-- modeles
alter table modeles add constraint modeles_unique1 unique(m_nom,m_couleur,m_type,m_memoire);

-- iphones
alter table iphones add constraint iphones_unique1 unique(i_nom);
alter table iphones add constraint iphones_unique2 unique(i_barcode);

-- fournisseurs
alter table fournisseurs add constraint fournisseurs_unique1 unique(f_nom);
alter table fournisseurs add constraint fournisseurs_unique2 unique(f_tel);

-- clients
alter table clients add constraint clients_unique1 unique(c_nom);
...
alter table acommandes add constraint acommandes_check1 check(ac_prix > 0);
...
alter table vcommandes add constraint vcommandes_check1 check(vc_prix > 0);
